...
NAVIGATION_TIMEOUT_MS = max(
    5000,
    _get_int("NAVIGATION_TIMEOUT_MS", max(PAGE_TIMEOUT_MS, 65000)),
)
ACTION_TIMEOUT_MS = max(
    1000,
    _get_int("ACTION_TIMEOUT_MS", min(PAGE_TIMEOUT_MS, 15000)),
)
STAGE_READY_TIMEOUT_MS = max(1000, _get_int("STAGE_READY_TIMEOUT_MS", 30000))
STAGE_READY_POLL_MS = max(250, _get_int("STAGE_READY_POLL_MS", 1000))
...
UNKNOWN_STAGE_EXP_BACKOFF = _get_bool("UNKNOWN_STAGE_EXP_BACKOFF", True)
UNKNOWN_STAGE_MAX_TOTAL_MS = max(
    UNKNOWN_STAGE_RETRY_BASE_MS,
    _get_int("UNKNOWN_STAGE_MAX_TOTAL_MS", 120000),
)

PROXY_PREFLIGHT_ENABLED = _get_bool("PROXY_PREFLIGHT_ENABLED", True)
PROXY_PREFLIGHT_TIMEOUT_MS = max(
    1000,
    _get_int("PROXY_PREFLIGHT_TIMEOUT_MS", 12000),
)
PROXY_PREFLIGHT_URLS = _parse_proxy_list(
    os.getenv(
        "PROXY_PREFLIGHT_URLS",
        "https://httpbin.org/ip,https://api.ipify.org?format=json",
    )
)
...
BLOCK_STREAK_BASE_SECONDS = max(60, _get_int("BLOCK_STREAK_BASE_SECONDS", 300))
BLOCK_STREAK_MAX_SECONDS = max(
    BLOCK_STREAK_BASE_SECONDS,
    _get_int("BLOCK_STREAK_MAX_SECONDS", 7200),
)
SUPPORT_ID_EXTRA_BACKOFF_SECONDS = max(
    60,
    _get_int("SUPPORT_ID_EXTRA_BACKOFF_SECONDS", 300),
)
